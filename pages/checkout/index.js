...
class CheckoutPage extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      'customer[first_name]': 'Nasa',
      'customer[last_name]': 'Azura',
      'customer[email]': 'nasa@azura.com',
      'customer[phone]': '',
      'customer[id]': null,
      'shipping[name]': 'Nasa Azura',
      'shipping[street]': 'Jalan Asia Afrika',
      'shipping[street_2]': '',
      'shipping[town_city]': 'Jakarta',
      'shipping[region]': 'ID-JK',
      'shipping[postal_zip_code]': '10540',
      'shipping[country]': 'ID',
      'billing[name]': '',
      'billing[street]': '',
      'billing[street_2]': '',
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      (hasDeliveryCountryChanged || hasDeliveryRegionChanged) &&
      this.props.checkout
    ) {
  ...
  generateToken() {
    const {cart, dispatchGenerateCheckout, dispatchGetShippingOptions} =
      this.props;
    const {'shipping[country]': country, 'shipping[region]': region} =
      this.state;
        ...
        .then((response) => {
          ...
          this.props
            ...
            .catch((error) => {
              ...
              this.props.stripe
                .handleCardAction(error.data.error.param)
                .then((result) => {
                ...
                >
                  ...
                  <div className='mb-5'>
                    ...
                    >
                      ...
                      />
                      <p>
                        Receive our news, restocking, good plans and news in
                        your mailbox! Rest assured, you will not be flooded, we
                        only send one newsletter per month approximately 🙂
                      </p>
                    </div>
                  ...
                  {checkout.collects && checkout.collects.billing_address && (
                    <>
                      ...
                      <div className='border border-color-gray400 mb-5'>
                        {billingOptions.map((value, index) => (
                          <label
                            key={index}
                            onClick={() =>
                              this.setState({selectedBillingOption: value})
                            }
                          ...
                          >
                            <Radiobox
                              checked={
                                this.state.selectedBillingOption === value
                              }
                              onClick={() =>
                      ...
                      {this.state.selectedBillingOption ===
                        'Use a different billing address' && (
                        <AddressForm
                          type='billing'
                          countries={this.state.countries}
